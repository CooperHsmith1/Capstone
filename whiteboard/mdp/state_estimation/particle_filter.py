...
class ParticleFilter(StateEstimator):
    ...
    def __init__(
        ...
        if proposal not in ("optimal", "bootstrap"):
            raise ValueError(
                f"proposal must be 'optimal' or 'bootstrap', got {proposal!r}."
            )
    ...
    def _update_optimal(self, measurement: torch.Tensor) -> torch.Tensor:
        ...
        L = torch.linalg.cholesky(
            Sigma + torch.eye(STATE_DIM, device=self.device) * 1e-12
        )
    ...
    def _reset_idx(
        ...
        particles = torch.zeros(
            k, self.num_particles, STATE_DIM, device=self.device
        )
        ...
        particles[..., POS_SLICE] = centre + spread[..., POS_SLICE] * self.cfg.initial_pos_std
    ...
    def _randn(self, shape: tuple[int, ...]) -> torch.Tensor:
        return torch.randn(
            *shape, device=self.device, generator=self._generator
        )

    def _randn_like(self, ref: torch.Tensor) -> torch.Tensor:
        return torch.randn(
            ref.shape, device=self.device, generator=self._generator
        )
