...
def pen_contact_reward(
    ...
    """Reward keeping the pen tip pressed against the board surface (X axis).

    Args:
        whiteboard_x: Env-local X coordinate of the board face, e.g. ``0.63``.
        asset_cfg:    SceneEntityCfg with site_names=("pen_tip",).
    """
    pen_pos = _pen_tip_pos_local(env, asset_cfg)                  # [B, 3] env-local
    x_error = torch.abs(pen_pos[:, 0] - whiteboard_x)            # [B]
    return torch.exp(-15.0 * x_error)
