...
def add_pen(spec: mujoco.MjSpec) -> mujoco.MjSpec:
  ...
  # Support XMLs that already include the pen attachment.
  if any(body.name == PEN_BODY for body in spec.bodies):
    return spec
...
def whiteboard_spec_fn(spec: mujoco.MjSpec) -> None:
  ...
  """
  if any(body.name == BOARD_BODY for body in spec.bodies):
    return
