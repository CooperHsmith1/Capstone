...
def unitree_g1_drawing_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Unitree G1 whiteboard drawing environment configuration.

  Starts from the base drawing env cfg (flat floor, fixed base, right arm
  only) and wires in the G1-specific robot asset plus play-mode overrides.
  ...
  # ------------------------------------------------------------------
  # Robot asset — G1 with the pen welded to the right wrist yaw link.
  # The MJCF is the edited g1_29dof_whiteboard.xml from the previous step.
  # ------------------------------------------------------------------
  cfg.scene.entities = {
    "robot": get_g1_robot_fixed_base_cfg(),
  }

  # Flat plane — the robot stands still, no terrain needed.
  cfg.sim.njmax = 300
  cfg.sim.mujoco.ccd_iterations = 50
  cfg.sim.contact_sensor_maxmatch = 64
  cfg.sim.nconmax = None
  ...
  # ------------------------------------------------------------------
  # Shared SceneEntityCfg objects (mirror drawing_env_cfg.py definitions
  # so reward params are always consistent).
  # ------------------------------------------------------------------
  ...
  # ------------------------------------------------------------------
  # Commands — random points on the board face, resampled every 4–8 s
  # early in training (reduce to 2–4 s once the policy converges).
  # ------------------------------------------------------------------
  cfg.commands["draw_target"] = DrawTargetCommandCfg(
    resampling_time_range=(4.0, 8.0),
    board_face_x=0.63,
    board_y_range=(-0.30, 0.30),
    board_z_range=(0.80, 1.40),
  )

  # ------------------------------------------------------------------
  # Rewards — explicit params so every function gets what it needs.
  # ------------------------------------------------------------------
  cfg.rewards["pen_tracking"] = RewardTermCfg(
  ...
  cfg.rewards["pen_contact"] = RewardTermCfg(
  ...
  # Negative weight: smooth_pen_motion_reward returns mean-squared velocity.
  cfg.rewards["smooth_pen_motion"] = RewardTermCfg(
  ...
  # ------------------------------------------------------------------
  # Terminations — no terrain, no out-of-bounds check needed.
  # ------------------------------------------------------------------
  cfg.terminations.pop("out_of_terrain_bounds", None)
  ...
  if play:
    ...
    cfg.observations["actor"].enable_corruption = False
    cfg.curriculum = {}
    # Hold each target longer in play so you can watch the arm reach it.
    cfg.commands["draw_target"] = DrawTargetCommandCfg(
