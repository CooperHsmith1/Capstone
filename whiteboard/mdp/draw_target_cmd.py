...
from .board import BOARD_FACE_X, TARGET_Y_RANGE, TARGET_Z_RANGE
...
class DrawTargetCommand(CommandTerm):
    ...
    def _update_command(self, env_ids: torch.Tensor | None = None) -> None:
        """No per-step evolution: the target is a fixed point until resampled.

        mjlab requires this signature. ``env_ids`` is None on the per-step
        update and holds the reset ids on reset; since the target does not
        advance between resamples there is nothing to scope.
        """
        return None
