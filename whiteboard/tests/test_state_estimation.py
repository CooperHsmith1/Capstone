...
def add_noise(
    ...
    g.manual_seed(seed)
    noisy = positions + torch.randn(
        positions.shape, generator=g, device=positions.device
    ) * sigma
    if outlier_rate > 0.0:
        mask = torch.rand(
            positions.shape[:2], generator=g, device=positions.device
        ) < outlier_rate
        spike = torch.randn(
            positions.shape, generator=g, device=positions.device
        ) * (sigma * outlier_scale)
...
def test_particle_filter_beats_kalman_on_outliers():
    ...
    for seed in range(5):
        meas = add_noise(
            truth, sigma, outlier_rate=0.06, outlier_scale=30.0, seed=seed
        )
...
def test_optimal_proposal_beats_bootstrap():
    ...
    boot = rmse(
        run_filter(ParticleFilter(B, cfg, num_particles=512, proposal="bootstrap", seed=0), meas, dt),
        truth,
    )
    opt = rmse(
        run_filter(ParticleFilter(B, cfg, num_particles=512, proposal="optimal", seed=0), meas, dt),
        truth,
    )
