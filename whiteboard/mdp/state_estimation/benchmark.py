...
def corrupt(
  ...
  if outlier_rate > 0.0:
    ...
    spike = torch.randn(truth.shape, generator=g, device=truth.device) * (
      sigma * outlier_scale
    )
  ...
  for regime_name, kwargs in regimes.items():
    for rep in range(repeats):
      ...
      builders = [
        ...
        lambda seed=seed: ParticleFilter(
          B,
          cfg,
      ...
      for build, label in zip(builders, labels, strict=True):
        est = build()
...
def format_table(results: list[BenchmarkResult]) -> str:
  ...
  lines: list[str] = [
    f"Aggregated over {n_reps} independent noise realisations (mean +/- std).",
  ]
